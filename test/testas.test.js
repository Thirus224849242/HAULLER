...
describe("Hauller Server API and WebSocket Tests", function () {
  ...
  it("13. Should broadcast message to room", function (done) {
  this.timeout(10000); // Optional: extend timeout to be safe
  ...
  client2.on("newMessage", (data) => {
    try {
      ...
      done(err); // So test fails clearly if expectation fails
    }
  });
  ...
  // ✅ Wait a moment to ensure both clients are in room
  setTimeout(() => {
  ...
  }, 500); // Adjust this value as needed depending on server responsiveness
});
